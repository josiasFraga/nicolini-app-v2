...
export const CenaSeparacaoCentral = (props) => {
    ...
    const loadPalletNumber = async () => {

        console.log('Carregando número do palete');

        try {
            let store_pallet_number = await AsyncStorage.getItem('pallet_number');

            if (store_pallet_number !== null) {
                setPalletNumber(store_pallet_number.replace('"','').replace('"',''));
            } else {                
                console.log('Nenhum número de palete encontrado');
            }
        }
        catch (e) {
            console.log(e);
    ...
    const finalizaSeparacao = () => {
		...
		if ( palletNumber === '' ) {
			...
			Alert.alert('Número do Pallet', 'O número do palete não foi gerado corretamente', [
				{
					text: 'OK',
					onPress: () => console.log('Cancel Pressed'),
					style: 'cancel',
